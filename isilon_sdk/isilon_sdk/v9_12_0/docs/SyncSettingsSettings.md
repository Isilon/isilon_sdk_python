...
**password_set** | **bool** | Indicates if a password is set for authentication. Password value is not shown with GET. | [optional] 
**preferred_rpo_alert** | **int** | If specified, display as default RPO Alert value for new policy creation via WebUI | [optional] 
**renegotiation_bytes** | **int** | Default for the number of bytes that are passed before a new TLS connection is 1TB. Setting to 0 will disable renegotiations based on bytes. | [optional] 
